...
forge's native tls stack lives in `std.net.tls` and `std.net.tls13`.

the current shape is:
- tls 1.3 only
- client and server handshakes in forge
- alpn
...
fn verify_internal(state: tls.ConnectionState) -> String:
    if state.negotiated_protocol != "forge.rpc":
        return "unexpected protocol"
    return ""

cfg := tls.client_config_with_ca_file("certs/root-ca.pem")!
cfg = cfg.with_alpn(["forge.rpc"]).with_verify_connection(verify_internal)
```
...
if it does send one, forge verifies it against the configured ca bundle.
