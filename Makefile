...
# built archive, not the source: the relocation model and the accessors.
check-tls-barriers:
	bash tooling/check_tls_barriers.sh target/release/libpith_runtime.a
...
MEMCHECK_CASES := \
	...
	tests/cases/test_map_fused_update \
...
# what `pith test` reports, pinned line for line. the json records are a
# contract with whatever ci reads them, and the tag runs pin which tests a
# selection runs, so both are compared against a golden rather than grepped.
test-runner-goldens: build test-runner-goldens-only
